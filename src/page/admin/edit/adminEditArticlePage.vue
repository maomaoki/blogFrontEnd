 ...
 * @param file
 */
function checkMdUploadImage(file: File) {
...
type callbackType = (urls: string[] | { url: string; alt: string; title: string }[]) => void;
/**
 *  md 上传 图片
 */
const onUploadImg = async (files: Array<File>, callback: callbackType) => {
	if (files.length < 0) {
		message.error("md上传图片错误")
		return
	}
	// 校验 文件类型 大小
	if (!checkMdUploadImage(files[0])) {
		return
	}
	
	// 上传 图片 拿到 回调
	// todo 上传 文件, 类型是文章md + 文章标题吧
	const result = await uploadPictureUsingPost({
		pictureCategory:
				PictureConstant.PICTURE_CATEGORY_ARTICLE_MD + "_" + editArticle.value.articleTitle
	}, {}, files[0])
	// @ts-ignore
	...
	//@ts-ignore
	const imageData: API.UploadPictureVo = result.data.data
	
	// 回调url
	callback(
			[
				{
					url: imageData.pictureUrl as string,
					alt: imageData.pictureName as string, title: imageData.pictureName as string
				}
			]
	)
