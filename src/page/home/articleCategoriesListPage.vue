<template>
  <div id="categoriesListPage" class="top-150">
      ...
      <div id="category-bar">
        ...
        <div class="catalog-list">
          <div class="catalog-list-item"
            ...
            <span @click="()=>router.push('/home')" v-if="item.name == '首页'">{{ '首页' }}</span>
            <span v-else
                  :class="item.name == route.params.title?'select':''"
                  @click="()=>router.push('/categories/'+item.name)">{{ item.name }}</span>
          </div>
        </div>
      ...
      <div class="article-sort">
        <div
          ...
          <div class="article-sort-item-info">
            <a class="article-sort-item-title">{{ item.articleTitle }}</a>
...
import {getArticleCategoryListUsingGet, pageArticleUsingPost} from "@/api/articleController.ts";
...
async function changePage(currentPage: number) {
  ...
  await doSearch({
    ...
    timeSortOrder: "desc",
    articleCategory: searchParams.value,
...
onBeforeRouteUpdate(async (to) => {
  searchParams.value = to.params.title as string
  pageNumber.value = 1
  // 刷新也需要 重置页码
